    ...
    try {
      ...
      for (const record of dto.records) {
        const caseResult = await queryRunner.query(
          `SELECT id, composite_key, organisation_id FROM registry.property_case
           WHERE UPPER(area_code) = UPPER($1)
             AND UPPER(valuation_number) = UPPER($2)
             AND deleted_at IS NULL`,
          [record.rawAreaCode, record.rawValuationNumber],
        );
