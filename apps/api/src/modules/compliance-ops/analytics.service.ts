...
export class AnalyticsService {
  ...
  async getArrearsReport(filters: { parish?: string; region?: string; financialYear?: string }, organisationId?: string) {
  ...
  async getCollectionsReport(period: Period, from: string, to: string, parish?: string, organisationId?: string) {
    ...
    if (organisationId) { p.push(organisationId); q += ` AND pc.organisation_id = $${p.length}`; }
    return this.db.query(q + ' GROUP BY 1,2,3 ORDER BY 1 DESC, amount_collected DESC', p);
  }

  async getOverallCollections(financialYear?: string, organisationId?: string) {
    ...
    const r = await this.db.query(
      `SELECT SUM(tb.amount_due) as total_levied,
       ...
       AND pc.organisation_id = COALESCE($2, pc.organisation_id)`,
      [fy, organisationId || null],
    );
    return { ...r[0], financialYear: fy };
  }

  async getDelinquencyReport(filters: { parish?: string; minYears?: number; minAmount?: number }, organisationId?: string) {
    let q = `
  ...
  async getForecast(monthsAhead: number = 6, organisationId?: string) {
  ...
  async getBottleneckAnalysis(organisationId?: string) {
